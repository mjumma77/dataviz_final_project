...
```{r warnings=FALSE,  fig.align="center", echo = FALSE,fig.width = 12}
...
```{r}
mpg_p <-ggplot(data = fuel_car_type[!(is.na(fuel_sel$trs_type)),], filter (fuel_sel,energy_type %in% c("Fossil")),
           mapping = aes(x = engine_displacement,
                          y = combined_mpg_ft1,
                          color = car_type)) + 
  geom_point(aes(size=engine_cylinders)) +
