...
```{r cars}
fuel_link <- "C:/Juma Data/Graduate Study/SU 2023 Courses/CAP 5735 Data Visualization/Projects/CAP 5735 Mini Project/Data/fuel.csv"
...
```{r warnings=FALSE,  fig.align="center", echo = FALSE,fig.width = 10}
mpg_p <-ggplot(data = fuel_sel[!(is.na(fuel_sel$trs_type)),], filter (fuel_sel,energy_type %in% c("Fossil")),
           mapping = aes(x = engine_displacement,
                          y = combined_mpg_ft1,
                          color = car_type)) + 
  geom_point(aes(size=engine_cylinders)) +
  ...
  labs(
    ...
    y = "Cobined Miles per Gallons",
    title = "Miles per Gallon per Number of Cylinders",
    subtitle = "Color and Size: Car Type and Engine Displacemenmt",
