...
loc_lakes <- "../data/Florida_Lakes/Florida_Lakes.shp"
...
fl_lakes %>%
  ...
  geom_sf_text(data = top_10_lakes_sf,
               aes(label = NAME), fill = NA, col = "grey20",
...
library(GGally)
library(broom)
west_house_mod <- lm(TOTAL.VALUE ~ GROSS.AREA + ROOMS + FLOORS,
                     data = west_rox)
hous_coefs <- tidy(west_house_mod, conf.int = TRUE) %>%
...
```{r}
pred_hous_value <- augment(west_house_mod, newdata = hous_new, se_fit = TRUE)
head(pred_hous_value)
```
...
ggplot(pred_hous_value,
       aes(x = GROSS.AREA, y = .fitted)) +
  geom_point(data = pred_hous_value, aes(col = ROOMS), alpha = 0.5) +
